...
namespace Dust
{
    ...
    {
        DUST_CORE_ERROR("GLFW Error: {0}, {1}", error, description);
    }
    ...
    void WindowsWindow::Init(const WindowProps& props)
    {
        ...
        DUST_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height)

        if (!s_GLFWInitialized)
        {
            int success = glfwInit();
            DUST_CORE_ASSERT(success, "Could not initialized GLFW!");
        ...
        DUST_CORE_ASSERT(status, "Failed to initialize Glad!");
        ...
        glfwSetMouseButtonCallback(m_Window, [](GLFWwindow* window, int button, int action, int mods)
        {
            ...
            switch (action)
           {
               case GLFW_PRESS:
                   ...
                   default:
                       DUST_WARN("Cannot parse mouse button event with action {0}", action);
               }
        });
